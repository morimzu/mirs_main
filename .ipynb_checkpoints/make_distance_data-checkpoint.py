...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        print("Data collection has already been completed for a distance of {}cm".format((i+1)*10+100))
        if i is not 10:
            print("Next Distance is {}cm".format((i+1)*10+10+100))
    ...
    for i, image in enumerate(images):
        ...
        for box in boxs:
            ...
            width += [box.width]
            height += [box.height]
            devi += [box.x_pos - image.shape[1]/2]
            count += 1
            #print(image.shape[1])
        if (i+1) % 5 == 0 and i is not 0:
            distance += 1
    df2 = pd.DataFrame(
